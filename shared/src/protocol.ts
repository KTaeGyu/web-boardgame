...
export interface ChatMessage {
  ...
  spectator?: boolean
  /**
   * 사람이 한 말이 아니라 방에 일어난 일인가(입장 같은).
   *
   * `playerId` 와 `name` 은 비어 있다. 누구의 말도 아니므로 이름을 붙이면 그 사람이
   * 한 말로 읽히고, 내 말·남의 말을 가르는 자리에도 서면 안 된다.
   */
  system?: boolean
}
...
export interface ServerToClientEvents {
  ...
  /**
   * 누가 무엇을 표현했나. **자리 위에 뜨는 것은 잠깐이라 상태에 남지 않는다** —
   * 뒤늦게 들어온 사람의 자리 위에 지난 감정이 뜨지 않는다.
   *
   * 다만 같은 표현이 대화에도 한 줄(chat:message)로 나간다. 자리 위의 것은 사건이고,
   * 대화에 남는 줄은 「무슨 말을 했나」의 기록이다 — 둘은 수명이 다르다.
   */
