...
export interface GameView {
  ...
  lockedTokens: number[]
  /**
   * 날아가는 중인 토큰이 **몇 ms 뒤에 풀리는가**. 붙박이처럼 시간으로 풀리지 않는 잠금은 없다.
   *
   * 화면이 이 값으로 스스로 잠금을 푼다. 서버의 「풀렸다」 재전송을 기다리면 한 번 더
   * 오가는 만큼 늦게 풀려, 토큰을 옮긴 뒤 다음 손이 굼떴다. 시각이 아니라 남은 길이로
   * 보내는 것은 서버와 화면의 시계가 어긋나 있어서다 — 받은 순간부터 세면 늦게 풀릴 뿐
   * 일찍 풀리지는 않는다(일찍 풀리면 눌러도 서버가 거절한다).
   */
  lockLeft: { token: number; ms: number }[]
  /**
