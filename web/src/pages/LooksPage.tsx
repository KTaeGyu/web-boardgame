 ...
 * 되돌릴 길이 없고(환불이 없다), 42골드짜리가 30골드짜리 옆 칸에 서 있다. 잘못 누르면
 * 42승어치가 그대로 나간다. 반대로 장착까지 물으면 차림을 맞춰 보는 동안 창이 계속 뜬다.
 *
...
export function LooksPage() {
  ...
  async function confirmBuy() {
    ...
    setPending(null)
    /*
     * **여기서 실패한 것은 장착뿐이고 구매는 이미 끝났다.** 장착 오류만 보이면 사람은
     * 아무 일도 없었다고 여기고 다시 누르는데, 그때는 「이미 보유한 아이템입니다」가 뜨고
     * 골드가 나갔다는 말은 어디에도 없다. 치른 값을 먼저 말한다.
     */
    if (failed) setNotice(`${item.price} 골드를 사용해 구매했습니다. 장착만 실패했습니다 — ${failed}`)
    else setNotice(`구매 완료 — ${item.price} 골드를 사용했습니다.`)
  }
