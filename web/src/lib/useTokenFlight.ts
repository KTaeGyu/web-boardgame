...
export function useTokenFlight(durationMs: number, layoutKey: string) {
  ...
  useLayoutEffect(() => {
    for (const [token, node] of nodes.current) {
       ...
       *
       * **`--z-verdict`(8) 자리다.** 층의 표는 web/src/styles.css 의 :root 에 있고,
       * 여기만 CSS 밖이라 숫자가 박혀 있다 — 표를 고치면 이 줄도 함께 봐야 한다.
       * 20 으로 두었더니 화면 위에 뜨는 것들(모달·대화)보다 높아, 대화창을 열어둔 채
       * 토큰을 집으면 칩이 창 위를 가로질러 날아갔다.
       */
