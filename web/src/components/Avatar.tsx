 ...
 * 꾸민 차림 한 덩이.
 *
 * 겹은 안쪽부터 **아바타 → 아바타배경 → 이펙트** 셋이고, 제일 바깥의 배너배경은
 * 줄 전체에 깔리는 것이라 여기 없다(그 줄을 그리는 쪽이 맡는다).
 *
 * **로그인한 사람만 꾸민다.** 게스트에게는 차림이 아예 오지 않으므로(`equipped`
 * 없음) 기본 네모로 그린다 — 빈자리로 두면 줄마다 높이가 달라진다.
 *
 * 그림은 `web/public/avatars/<이름>.svg` 에 있다. 이름표로 찾아 그리므로, 파일이
 * 없거나 이름이 낡았으면 기본 네모로 되돌아간다(`onError`) — 꾸미기 하나 때문에
 * 깨진 그림 자리가 남지는 않게 한다.
 */
