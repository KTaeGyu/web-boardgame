 ...
 */
export function Token({ value, round, locked = false, onClick, settled = false, innerRef }: Props) {
  const classes = [
    'token',
  ...
  if (settled || !onClick) {
    return (
      <span className={classes} ref={innerRef} aria-label={`${round}라운드 ${value}번 토큰`}>
        {value}
      </span>
    )
  }
